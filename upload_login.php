...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($username) || empty($password)) {
        ...
        if ($user && password_verify($password, $user['password'])) {
            ...
            if ($role == 'Student') {
                header("Location: /SORS_ex/Student/Upload_FilesSTUDENT.php"); // Redirect to student upload page
            } else {
                header("Location: /SORS_ex/Teacher/Upload_FilesTEACHER.php"); // Redirect to teacher upload page
            }
            exit;
        } else {
